...
class CocoDetection(data.Dataset):
    ...
    def __init__(self, root, dataset="train", transforms=None, years="2022"):
        ...
        data_classes = dict([(v["id"], v["name"]) for k, v in self.coco.cats.items()])  # 读取我定义的类
        max_index = max(data_classes.keys())  # 90
        # 将缺失的类别名称设置成N/A
        coco_classes = {}
        for k in range(1, max_index + 1):  # 从1开始，0是背景
            if k in data_classes:
                coco_classes[k] = data_classes[k]
            else:
                coco_classes[k] = "N/A"

        if dataset == "train":  # dataset是传入的值
            json_str = json.dumps(coco_classes, indent=4)  # 把类写入json文件
    ...
    def __getitem__(self, index):
        ...
        img = Image.open(os.path.join(self.img_root, path)).convert('RGBA')
        img2 = Image.open(os.path.join(self.img_root, path)).convert('RGB')
        # print(type(img))
        # img1 = np.array(img)
        # print(img1.shape)
        # img1 = img1[:, :, 0:3]
...
if __name__ == '__main__':
    train = CocoDetection("./data/coco2022", dataset="train")
    print(len(train))
    t = train[0]
