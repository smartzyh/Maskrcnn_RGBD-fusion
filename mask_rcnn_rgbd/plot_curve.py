...
def plot_loss_and_lr(train_loss, learning_rate):
    try:
        ...
        fig.savefig('./results/onnx/coco_all/loss_and_lr{}.png'.format(datetime.datetime.now().strftime("%Y%m%d-%H%M%S")))
        plt.close()
...
def plot_map(mAP):
    try:
        ...
        plt.xlabel('epoch')
        plt.ylabel('mAP')
        plt.title('Eval mAP101')
        plt.xlim(0, len(mAP))
        plt.legend(loc='best')
        plt.savefig('./results/onnx/coco_all/mAP{}.png'.format(datetime.datetime.now().strftime("%Y%m%d-%H%M%S")))#加上时间
        plt.close()
