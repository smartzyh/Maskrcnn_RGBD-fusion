...
def create_model(num_classes, load_pretrain_weights=True):
    ...
    backbone = resnet50_fpn_backbone(pretrain_path="./pretrain_weights/resnet101_cut.pth", trainable_layers=5)
    ...
    if load_pretrain_weights:
        ...
        weights_dict = torch.load("./pretrain_weights/maskrcnn_resnet50_fpn_coco_cut.pth", map_location="cpu")
...
def main(args):
    ...
    # 用来保存coco_info的文件 我重新定义了文件夹
    now = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    det_results_file = f"./results/onnx/coco_all/det_results{now}.txt"
    seg_results_file = f"./results/onnx/coco_all/seg_results{now}.txt"
    ...
    # 单独保存训练参数，并传值到plot_curve.py中，绘制图片
    train_loss = []   
    learning_rate = []
    val_map = []
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % 5 == 0:
            ...
            torch.save(save_files, "./save_weights/onnx/coco_all/model_{}.pth".format(epoch))
        torch.save(model, "./save_weights/onnx/coco_all/model_onnx{}.pth".format(epoch))

    # plot loss and lr curve 画图
...
if __name__ == "__main__":
    ...
    # 训练数据集的根目录
    parser.add_argument('--data-path', default=r'E:\ML\vsCode\pytorch\deep-learning-for-image-processing-master\pytorch_object_detection\mask_rcnn_rgbd_resres\data\coco_all', help='dataset')
    # 检测目标类别数(不包含背景)
    ...
    parser.add_argument('--lr-steps', default=[16, 22, 35], nargs='+', type=int,
