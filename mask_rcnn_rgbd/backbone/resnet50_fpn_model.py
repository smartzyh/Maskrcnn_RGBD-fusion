...
class ResNet(nn.Module):
    ...
    #     if
#加加加加
# def init_weights(pretrained = True):
#     if pretrained == True:
#         model = ResNet(block=Bottleneck,num_classes=4, blocks_num=[3, 4, 6, 3])

#         model_dict = model.state_dict()
#         print(model_dict)
#         for key in list(model_dict.keys()):
#             if 'conv1.weight' in key : del model_dict[key]
...
def resnet50_fpn_backbone(pretrain_path="",
    ...
    resnet_backbone = ResNet(Bottleneck, [3, 4, 23, 3],
                             include_top=False,
                             norm_layer=norm_layer)  # 默认训练101，从这里载入模型
